...
interface Props {
  ...
  row?: FixtureRow;
  onSubmit: (row: FixtureRow) => void;
}

export default function EditFixtureDialog({ open, onOpenChange, row, onSubmit }: Props) {
  const [form, setForm] = useState<FixtureRow | undefined>(row);
  ...
  const set = <K extends keyof FixtureRow>(k: K, v: FixtureRow[K]) =>
    setForm(p => (p ? { ...p, [k]: v } : p));
  ...
  useEffect(() => {
    if (form?.type) {
      import('@/@data/sidebar').then(sidebar => {
        const sidebarId = sidebar.default.find(s => s.title === form.type)?.id || undefined;
        console.log('🚀 ~ EditFixtureDialog ~ sidebarId:', sidebarId);
        if (sidebarId) set('sidebarId', sidebarId);
      });
    }
  }, [form?.type]);
